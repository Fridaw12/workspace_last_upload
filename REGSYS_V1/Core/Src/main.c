...
void liten_sjekk_17(void){
	memcpy(&TxData,   &FELICESPERDIDOSNR2, 1); // Kopierer 1 byte, sum_bidrag_int8.hhf til første byte i TxData
	memcpy(&TxData[1],&FELICESPERDIDOSNR2, 1); // Kopierer 1 byte, sum_bidrag_int8.hhb til andre byte i TxData
	memcpy(&TxData[2],&FELICESPERDIDOSNR2, 1); // ..
	memcpy(&TxData[3],&FELICESPERDIDOSNR2, 1);
	memcpy(&TxData[4],&FELICESPERDIDOSNR2, 1);
	memcpy(&TxData[5],&FELICESPERDIDOSNR2, 1);
	memcpy(&TxData[6],&FELICESPERDIDOSNR2, 1);
	memcpy(&TxData[7],&FELICESPERDIDOSNR2, 1);
}

void liten_sjekk_C(void){
	memcpy(&TxData,   &FELICESPERDIDOSNR2, 1); // Kopierer 1 byte, sum_bidrag_int8.hhf til første byte i TxData
	memcpy(&TxData[1],&FELICESPERDIDOSNR2, 1); // Kopierer 1 byte, sum_bidrag_int8.hhb til andre byte i TxData
	memcpy(&TxData[2],&FELICESPERDIDOSNR2, 1); // ..
	memcpy(&TxData[3],&FELICESPERDIDOSNR2, 1);
	memcpy(&TxData[4],&operator_referanse_int.jag, 1);
	memcpy(&TxData[5],&operator_referanse_int.svai, 1);
	memcpy(&TxData[6],&operator_referanse_int.hiv, 1);
	memcpy(&TxData[7],&operator_referanse_int.gir, 1);
}
  ...
  if((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
  {
    ...
    switch (RxHeader.Identifier) { // Leser ID på motatt melding. Casene må lages ut fra ID-er enn mottar. (Sjekk "Interface-Overview" under "Overordnet prosjekt")
		...
		case 33:  // STYREPAKKE THRUSTER
			//MOTTATT_THRUST = 1; // Sett flagg for mottagelse.
...
//			// Tx[B0,B1,B2,B3,B4,B5,B6,B7]
			...
			//memcpy(&operator_manpu_int, &RxData[4], 1); // B4
			//memcpy(&operator_manpu_int, &RxData[5], 1); // B5
